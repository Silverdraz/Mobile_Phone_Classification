...
resource null_resource ecr_image {
  ...
  provisioner "local-exec" {
    working_dir = "../"
     command = "docker build --provenance=false -t ${aws_ecr_repository.repo.repository_url}:${var.ecr_image_tag} ."   
  }

  provisioner "local-exec" {
     command = "docker push ${aws_ecr_repository.repo.repository_url}:${var.ecr_image_tag}"   
  }
}
